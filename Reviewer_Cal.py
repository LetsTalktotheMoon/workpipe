...
WEIGHTS = {
    ...
    "r1_writing_standard":   0.12,
    "r2_jd_fitness":         0.22,
    "r3_overqualification":  0.05,
    "r4_rationality":        0.23,
    "r5_logic":              0.08,
    "r7_ecosystem":          0.07,
...
REVISION_THRESHOLD = 93.0  # 综合分 < 93 触发 revision
FAIL_THRESHOLD = 85.0      # 综合分 < 85 判定 fail
HIGH_COUNT_REVISION = 3    # high findings >= 3 触发 revision
...
def score_dimension(findings: list[Finding]) -> float:
    ...
    规则：
    - 起点 10 分
    - 每条 critical: -4, high: -2, medium: -0.8, low: -0.3
...
def aggregate(reviewer_output: ReviewerRawOutput) -> AggregatedResult:
    ...
    if severity_counts["critical"] > 0 or weighted_score < FAIL_THRESHOLD:
