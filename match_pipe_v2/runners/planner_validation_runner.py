...
def _plan_flow(
    *,
    ...
    prompt = registry.render_view(
        "prompt_planner_user",
        context=build_planner_context(jd, mode=mode, matcher_packet=matcher_packet, starter_resume_md=starter_resume_md),
    )
    payload = client.call_json(
        prompt,
        model=planner_model,
        cache_key=client.make_cache_key(jd.jd_id or "unknown", jd.company or "", jd.title or "", cache_key_suffix),
        system=registry.render_block("planner_system"),
    )
    return payload, _estimate_tokens(prompt) + _estimate_tokens(registry.render_block("planner_system")), _estimate_tokens(json.dumps(payload, ensure_ascii=False))
...
def _review_direct_or_written(
    *,
    ...
    while not review.passed and review.needs_revision and writer_rounds < 2:
        writer_rounds += 1
        if is_bytedance_target_company(jd.company):
            revision_prompt = registry.render_view(
...
def _run_no_starter_planner(
    ...
    *,
    ...
    if is_bytedance_target_company(jd.company):
        writer_prompt = registry.render_view(
            "prompt_planner_writer_full_bytedance",
            context=build_planner_writer_context(jd, planner_payload=planner_payload),
        )
    else:
        writer_prompt = registry.render_view(
    ...
    *,
    ...
    if planner_payload.get("decision") == "reject_starter":
        if is_bytedance_target_company(jd.company):
            writer_prompt = registry.render_view(
                "prompt_planner_writer_full_bytedance",
                context=build_planner_writer_context(jd, planner_payload=planner_payload, starter_resume_md="", matcher_packet=matcher_packet),
            )
        else:
            writer_prompt = registry.render_view(
                "prompt_planner_writer_full",
