...
from match_pipe_v2.frozen_teacher import frozen_teacher_manifest
from match_pipe_v2.loader import load_job_documents
from match_pipe_v2.matcher import MatchEngine
from match_pipe_v2.starter_selector import StarterSelector
...
def _review_prompt_tokens(jd: JDProfile, resume_md: str, review_summary) -> tuple[int, int]:
    if is_bytedance_target_company(jd.company):
        prompt = registry.render_view("prompt_reviewer_full_bytedance", context=build_reviewer_context(resume_md, jd, review_scope="full"))
    else:
        prompt = registry.render_view("prompt_reviewer_full", context=build_reviewer_context(resume_md, jd, review_scope="full"))
    prompt_tokens = _estimate_tokens(registry.render_block("reviewer_system")) + _estimate_tokens(prompt)
...
def _review_with_revision(
    *,
    ...
    review = reviewer.review(
        ...
        mode="full",
        prompt_override=(
            registry.render_view("prompt_reviewer_full_bytedance", context=build_reviewer_context(initial_resume_md, jd, review_scope="full"))
            if is_bytedance_target_company(jd.company)
            else registry.render_view("prompt_reviewer_full", context=build_reviewer_context(initial_resume_md, jd, review_scope="full"))
        ),
        system_prompt_override=registry.render_block("reviewer_system"),
    )
    ...
    while not review.passed and review.needs_revision and rewrite_rounds < 1:
        ...
        revised_md = writer.revise(
            ...
            system_prompt_override=registry.render_block("upgrade_revision_system"),
        )
        revised_review = reviewer.review(
            ...
            mode="full",
            prompt_override=(
                registry.render_view("prompt_reviewer_full_bytedance", context=build_reviewer_context(revised_md, jd, review_scope="full"))
                if is_bytedance_target_company(jd.company)
                else registry.render_view("prompt_reviewer_full", context=build_reviewer_context(revised_md, jd, review_scope="full"))
            ),
            system_prompt_override=registry.render_block("reviewer_system"),
        )
...
def _run_no_starter(writer: MasterWriter, reviewer: UnifiedReviewer, jd: JDProfile) -> FlowResult:
    ...
    if is_bytedance_target_company(jd.company):
        prompt = registry.render_view("prompt_writer_generate_bytedance", context=build_writer_context(jd))
    else:
        prompt = registry.render_view("prompt_writer_generate", context=build_writer_context(jd))
    resume_md, _ = writer.write_from_prompt(
        prompt,
        jd=jd,
        system_prompt=registry.render_block("writer_system"),
    ...
    result = _review_with_revision(
        ...
        revision_prompt_builder=lambda current_md, review: (
            registry.render_view(
                "prompt_upgrade_revision_bytedance",
                context=build_upgrade_context(
                    current_md,
                    review.__dict__ | {
                        "scores": {
                            key: {
...
def _run_old_match(
    ...
    *,
    ...
    if is_bytedance_target_company(jd.company):
        view_id = "prompt_retarget_old_match_bytedance"
    else:
        view_id = "prompt_retarget_same_company" if same_company else "prompt_retarget_old_match"
    prompt = registry.render_view(view_id, context=ctx)
    resume_md = writer.revise(
        seed_resume_md,
        prompt,
        jd,
        system_prompt_override=registry.render_block("strict_revision_system"),
    )
    result = _review_with_revision(
        writer=writer,
        reviewer=reviewer,
...
def _run_new_dual_channel(
    ...
    *,
    ...
    if is_bytedance_target_company(jd.company):
        prompt = registry.render_view("prompt_dual_channel_full_bytedance", context=ctx)
    else:
        prompt = registry.render_view("prompt_dual_channel_full", context=ctx)
    resume_md = writer.revise(
        seed_resume_md,
        prompt,
        jd,
