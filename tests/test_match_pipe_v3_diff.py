...
def test_core_views_and_report() -> None:
    ...
    new = registry.render_view("prompt_retarget_old_match", context=build_retarget_context(jd, seed_resume_md=seed_resume_md))
    ...
    new = registry.render_view(
        ...
        context=build_upgrade_context(resume_md, review_result, tech_required=jd.tech_required, target_company=jd.company),
    )
    ...
    ctx = build_retarget_context(jd, seed_resume_md=seed_resume_md)
    ...
    # --- ByteDance coverage ---
    jd_bd = _make_jd()
    jd_bd.company = "ByteDance"

    # writer_generate bytedance
    old = build_match_pipe_master_writer_prompt(jd_bd)
    new = registry.render_view("prompt_writer_generate", context=build_writer_context(jd_bd))
    if diff := _diff_report(old, new, "prompt_writer_generate_bytedance"):
        reports.append(diff)

    # reviewer_full bytedance
    old = build_match_pipe_unified_review_prompt(resume_md, jd_bd, review_scope="full")
    new = registry.render_view("prompt_reviewer_full", context=build_reviewer_context(resume_md, jd_bd, review_scope="full"))
    if diff := _diff_report(old, new, "prompt_reviewer_full_bytedance"):
        reports.append(diff)

    # retarget_old_match bytedance
    old = build_match_pipe_seed_retarget_prompt(
        seed_resume_md,
        jd_bd,
        seed_label="seed_label",
    ...
    new = registry.render_view("prompt_retarget_old_match", context=build_retarget_context(jd_bd, seed_resume_md=seed_resume_md))
    if diff := _diff_report(old, new, "prompt_retarget_old_match_bytedance"):
        reports.append(diff)

    # retarget_same_company bytedance
    old = build_match_pipe_seed_retarget_prompt(
        seed_resume_md,
        jd_bd,
        seed_label="seed_label",
        route_mode="retarget",
        top_candidate={"same_company": True, "missing_required": []},
    )
    ctx = build_retarget_context(jd_bd, seed_resume_md=seed_resume_md, top_candidate={"same_company": True, "missing_required": []})
    new = registry.render_view("prompt_retarget_old_match", context=ctx)
    if diff := _diff_report(old, new, "prompt_retarget_old_match_same_company_bytedance"):
        reports.append(diff)

    # upgrade_revision bytedance
    old = build_match_pipe_upgrade_revision_prompt(
        resume_md,
        review_result,
        tech_required=jd_bd.tech_required,
        jd_title=jd_bd.title,
        target_company=jd_bd.company,
    )
    new = registry.render_view(
        "prompt_upgrade_revision",
        context=build_upgrade_context(resume_md, review_result, tech_required=jd_bd.tech_required, target_company=jd_bd.company),
    )
    if diff := _diff_report(old, new, "prompt_upgrade_revision_bytedance"):
        reports.append(diff)

    # planner_writer_full bytedance
    old = build_match_pipe_writer_prompt_from_planner(
        jd=jd_bd,
        planner_payload=planner_payload,
        starter_resume_md=starter_resume_md,
    ...
    new = registry.render_view(
        "prompt_planner_writer_full",
        context=build_planner_writer_context(jd_bd, planner_payload=planner_payload, starter_resume_md=starter_resume_md, matcher_packet=matcher_packet),
    )
    if diff := _diff_report(old, new, "prompt_planner_writer_full_bytedance"):
        reports.append(diff)

    # planner_revision_full bytedance
    old = build_match_pipe_writer_revision_prompt(current_resume_md, review, jd_bd, planner_payload)
    new = registry.render_view(
        "prompt_planner_revision_full",
        context=build_planner_revision_context(current_resume_md, review, planner_payload, jd=jd_bd),
    )
    if diff := _diff_report(old, new, "prompt_planner_revision_full_bytedance"):
        reports.append(diff)
    ...
    if reports:
        ...
        raise AssertionError(
            f"Detected {len(reports)} view/block diff(s) between old prompt_library and new v3 PromptRegistry. "
            "See stdout for detailed diff report."
        )
