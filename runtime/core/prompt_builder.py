...
BYTEDANCE_EVIDENCE_POOL_LONG = (
    "DiDi 全职经历、Temu 全职经历、Georgia Tech Computer Science 的课程项目/课程作业"
)
...
→ 此经历 bullet 将使用的完整技术列表: [最终列表]"""
MASTER_PLAN_GT_SECTION = """### Georgia Tech CS coursework/projects（补足 SWE / systems / backend 缺口）
...
- 若简历仍引用了 TikTok / ByteDance intern，视为 critical，并要求删除后仅用 DiDi / Temu / Georgia Tech CS coursework-projects 重写
"""
...
def _target_specific_context_block(jd_or_company) -> str:
    ...
    return _join_lines(
        [
            ...
            "- 当 JD 需要更强的软件工程或系统实现信号时，优先把 Georgia Tech CS coursework/projects 提升为主要技术证据，而不是把缺口强行塞回 ByteDance/TikTok intern",
...
def _build_master_plan_template(*, bytedance_mode: bool) -> str:
    ...
    if not bytedance_mode:
        sections = [MASTER_PLAN_TIKTOK_SECTION, MASTER_PLAN_DIDI_SECTION, MASTER_PLAN_TEMU_SECTION]
    return "\n\n".join(sections)
...
def build_candidate_context(company_name: str = "") -> str:
    ...
    for edu in CANDIDATE_FRAMEWORK["education"]:
        ...
        if edu["id"] == "gt_mscs":
            ...
            if is_bytedance_target_company(company_name):
                lines.append(
                    "  → ByteDance 目标岗位中，Georgia Tech CS coursework/projects 可以作为主要软件工程证据来源。"
                )
    lines.append("")
    ...
    if is_bytedance_target_company(company_name):
        ...
        lines.append("- 目标岗位只能使用 DiDi、Temu 和 Georgia Tech CS coursework/projects 作为证据池")
